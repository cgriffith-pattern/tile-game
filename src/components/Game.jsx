...
export default class Game extends React.Component {
  ...
  shuffle(array){
    var ran, temp, curr = (this.state.width * this.state.width) - 1;
  ...
  ordered(array){
    ...
    var i, length = (this.state.width * this.state.width) - 1;
  ...
  solvable(array){
    ...
    var inversions = 0, i, j; //inversion counter, indexes
    ...
    for (i = 0; i < (width * width) - 1; i++){
      for (j = i + 1; j < (width * width); j++){
        ...
        if (array[(j).toString()] !== "" && array[(i).toString()] !== "" && parseInt(array[(i).toString()]) > parseInt(array[(j).toString()])){
          inversions += 1;
        }
      }
    }
    ...
    if (width % 2 === 1){ //if width is odd
      if (inversions % 2 === 0){ //number of inversions is even
        return true;
      }
    }
    //if width is even, then
    else if (parseInt((blankPos / width) % 2) === 1){ //if blank is on an odd row
      if (inversions % 2 === 0){ //number of inversions is even
        return true;
      }
    }
    else { //if blank is on an even row
      if (inversions % 2 === 1){ //number of inversions is odd
        return true;
      }
    }
  ...
  handleClick = (tileNumber) => {
    ...
    if ((Math.abs(bk_num - tk_num) === 1) || (Math.abs(bk_num - tk_num) === width)) {
      //check for edge violation
      if (((bk_num % width !== 0 || tk_num % width !== width - 1) 
        && (tk_num % width !== 0 || bk_num % width !== width - 1))){

        tiles[blank_key] = tileNumber;
        tiles[tile_key] = "";
        //update state
        this.setState({solved: (this.ordered(this.state.tiles)), moves: (this.state.moves + 1), tiles: tiles});
      }
    }
  };
  ...
  renderCols(tileNumber){
    ...
    for (i = 0; i < height; i++){
      cols.push(<div className="GameRow"> {this.renderRows(tNum.toString())} </div>);
      tNum += height;
    }
  ...
  render() {
    return (
       ...
       <div>
        <button onClick={()=>{this.nextPuzzle();}}> next puzzle </button>
       </div>
       <div>
        <button onClick={()=>{this.reset();}}>reshuffle</button>
       </div>
       <div>
